<!--Moderator Login.php-->
<!--Moderator login into the system-->
<?php
    ...
    if (isset($_POST["submit"])) {
        ...
        // Query the moderators table
        $stmt = $conn->prepare("SELECT moderatorID, name, email, password, status FROM moderators WHERE email = ?");
            ...
            if ($result->num_rows > 0) {
                ...
                if (password_verify($password, $row['password']) || $password === $row['password']) {
                    ...
                    if ($row['status'] === 'active') {
                        ...
                        // If status is not 'active', they cannot log in
                        $message = "<p class='msg-warning'>Your account is not active. Please contact the administrator.</p>";
                    }
                ...
                $message = "<p class='msg-error'>Moderator account not found with that email.</p>";
            }
            $stmt->close();
        } else {
...
<body class="login-body">
    <div class="box">
        <h2 class="h2">Moderator Log In</h2>
        ...
        <form action="ModeratorLogin.php" method="POST">
            ...
            <div class="links center-links">
                <a href="StudentLogin.php">Student Login</a> | <a href="AdminLogin.php">Admin Login</a>
            </div>
        </form>
    </div>
</body>
